...
export function migrateDatabase(
  ...
  for (const migration of available) {
    ...
    try {
      ...
      if ((database.pragma('foreign_key_check') as unknown[]).length) throw new Error('Migration failed foreign key validation')
      if (migration.version === 22) database.pragma('defer_foreign_keys = OFF')
      database
        .prepare(
...
export function migrateActiveV2DatabaseFrom12To15(
  ...
  const available = discoverMigrations(migrationsPath)
  const availableVersion = available.at(-1)?.version ?? 0
  if (availableVersion !== 22) {
    throw new Error(`Migration 0022 must be the repository tip, found ${availableVersion}`)
  }
  if (!databaseTableExists(database, '_schema_migrations')) {
  ...
  if (
    ...
    pending[6]?.version !== 19 || pending[7]?.version !== 20 || pending[8]?.version !== 21 || pending[9]?.version !== 22
  ) {
    issues.push('The only permitted pending migrations are 0013 through 0022')
  }
  if (issues.length > 0) {
    throw new Error(issues.join('; '))
  }
  ...
  for (const migration of pending) {
    ...
    if (migration.version === 22) {
      if ((database.pragma('foreign_key_check') as unknown[]).length) throw new Error('Event entry foreign keys invalid')
...
export function migrateActiveV2ProgramCatalogFrom14To15(
  ...
  if (!history.ready || history.availableVersion !== 22) throw new Error('The catalog upgrade requires exact schema 14 and repository tip 22')
  ...
  return { ...final, applied: [15, 16, 17, 18, 19, 20, 21, 22], previousVersion: 14 }
}
...
export function migrateActiveV2InteractionsFrom15To16(database: SqliteDatabase, migrationsPath: string, now: () => Date = () => new Date()): ActiveV2MigrationResult {
  ...
  if (!history.ready || history.availableVersion !== 22) throw new Error('Interaction upgrade requires exact schema 15 and tip 22')
  ...
  return { ...final, applied: [16, 17, 18, 19, 20, 21, 22], previousVersion: 15 }
}
...
export function migrateActiveV2GiftExperienceFrom16To17(database: SqliteDatabase, migrationsPath: string, now: () => Date = () => new Date()): ActiveV2MigrationResult {
  ...
  if (!history.ready || history.availableVersion !== 22) throw new Error('Gift experience upgrade requires exact schema 16 and tip 22')
  ...
  return { ...final, applied: [17, 18, 19, 20, 21, 22], previousVersion: 16 }
}
...
export function migrateActiveV2ProgramCreditsFrom17To18(database: SqliteDatabase, migrationsPath: string, now: () => Date = () => new Date()): ActiveV2MigrationResult {
  ...
  if (!history.ready || history.availableVersion !== 22) throw new Error('Programme credits upgrade requires exact schema 17 and tip 22')
  ...
  return { ...final, applied: [18, 19, 20, 21, 22], previousVersion: 17 }
}
...
export function migrateActiveV2LiveInteractionsFrom18To19(
  ...
  if (!history.ready || history.availableVersion !== 22) throw new Error('Live interaction upgrade requires exact schema 18 and tip 22')
  ...
  return { ...final, applied: [19, 20, 21, 22], previousVersion: 18 }
}
...
export function migrateActiveV2AwardsFrom19To20(database: SqliteDatabase, migrationsPath: string, now: () => Date = () => new Date()): ActiveV2MigrationResult {
  ...
  if (!history.ready || history.availableVersion !== 22) throw new Error('Awards upgrade requires exact schema 19 and tip 22')
  ...
  return { ...final, applied: [20, 21, 22], previousVersion: 19 }
}
...
export function migrateActiveV2ProgramRankingFrom20To21(database: SqliteDatabase, migrationsPath: string, now: () => Date = () => new Date()): ActiveV2MigrationResult {
  ...
  if (!history.ready || history.availableVersion !== 22) throw new Error('Programme ranking upgrade requires exact schema 20 and tip 22')
  ...
  return { ...final, applied: [21, 22], previousVersion: 20 }
}

/** Caller owns the stopped-service check, verified backup and write transaction. */
export function migrateActiveV2EventEntryFrom21To22(database: SqliteDatabase, migrationsPath: string, now: () => Date = () => new Date()): ActiveV2MigrationResult {
  if (!database.inTransaction) throw new Error('Event entry upgrade requires an existing transaction')
  const protocol = database.prepare('SELECT active_protocol_version AS version, activation_state AS state FROM protocol_runtime WHERE id = 1').get() as { version: string; state: string } | undefined
  ...
  if (!history.ready || history.availableVersion !== 22) throw new Error('Event entry upgrade requires exact schema 21 and tip 22')
  ...
  // Recreated parent keys pass the complete FK check. Clear SQLite's deferred
  // DROP bookkeeping before commit; foreign_keys itself remains enabled.
  database.pragma('defer_foreign_keys = OFF')
  ...
  return { applied: [22], previousVersion: 21, currentVersion: 22, availableVersion: 22 }
}
