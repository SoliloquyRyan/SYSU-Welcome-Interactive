...
export function migrateActiveV2GuestsFrom22To23(database: SqliteDatabase, migrationsPath: string, now: () => Date = () => new Date()): ActiveV2MigrationResult {
  if (!database.inTransaction) throw new Error('Guest entry upgrade requires an existing transaction')
  const protocol = database.prepare('SELECT active_protocol_version AS version, activation_state AS state FROM protocol_runtime WHERE id = 1').get() as { version: string; state: string } | undefined
