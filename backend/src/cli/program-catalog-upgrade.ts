...
import { upgradeV2ProgramCatalogFrom14To15 } from '../db/v2-foundation.js'
...
if (!backup || !process.env.DEMO_DATABASE_PATH?.trim() || !process.env.DEMO_SEED_MANIFEST_PATH?.trim()) {
  ...
  console.error('节目目录升级需要 --backup <全新备份路径>，先停止服务再使用 --confirm V2_SERVICES_STOPPED。')
  process.exitCode = 1
} else {
  ...
  try {
    const result = await upgradeV2ProgramCatalogFrom14To15(database, {
      migrationsPath: config.migrationsPath, manifestPath: config.seedManifestPath,
      participantCount: config.seedParticipantCount, backupPath: path.resolve(backup),
      confirmation: argument('--confirm'),
    })
    console.log(`节目目录升级完成：schema=${result.previousSchemaVersion} -> ${result.schemaVersion}，resetEpoch=${result.resetEpoch}，备份 SHA-256=${result.backupSha256}`)
  } catch (error) { reportCliFailure('节目目录 schema 14→15 升级', error) }
