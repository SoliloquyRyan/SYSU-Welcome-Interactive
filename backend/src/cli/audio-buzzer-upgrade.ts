...
import { upgradeV2AudioFrom23To24 } from '../db/v2-foundation.js'
...
if (!backup || !process.env.DEMO_DATABASE_PATH?.trim() || !process.env.DEMO_SEED_MANIFEST_PATH?.trim()) {
  ...
  console.error('互动音频抢答升级需要 --backup <全新备份路径>，先停止本项目服务再使用 --confirm V2_SERVICES_STOPPED。')
  process.exitCode = 1
} else {
  ...
  try {
    const result = await upgradeV2AudioFrom23To24(database, {
    ...
    console.log(`互动音频抢答升级完成：schema=${result.previousSchemaVersion} -> ${result.schemaVersion}，resetEpoch=${result.resetEpoch}，备份 SHA-256=${result.backupSha256}`)
  } catch (error) {
    reportCliFailure('互动音频抢答 schema 23→24 升级', error)
  } finally {
