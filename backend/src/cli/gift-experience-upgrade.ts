...
import { upgradeV2GiftExperienceFrom16To17 } from '../db/v2-foundation.js'
...
if (!backup || !process.env.DEMO_DATABASE_PATH?.trim() || !process.env.DEMO_SEED_MANIFEST_PATH?.trim()) {
  ...
  console.error('礼物体验升级需要 --backup <全新备份路径>，先停止服务再使用 --confirm V2_SERVICES_STOPPED。')
  process.exitCode = 1
} else {
  ...
  try {
    const result = await upgradeV2GiftExperienceFrom16To17(database, {
    ...
    console.log(`礼物体验升级完成：schema=${result.previousSchemaVersion} -> ${result.schemaVersion}，resetEpoch=${result.resetEpoch}，备份 SHA-256=${result.backupSha256}`)
  } catch (error) {
    reportCliFailure('礼物体验 schema 16→17 升级', error)
  } finally {
