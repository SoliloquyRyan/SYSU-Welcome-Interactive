...
import { upgradeV2GuestsFrom22To23 } from '../db/v2-foundation.js'
...
if (!backup || !process.env.DEMO_DATABASE_PATH?.trim() || !process.env.DEMO_SEED_MANIFEST_PATH?.trim()) {
  ...
  console.error('游客与正式轮次升级需要 --backup <全新备份路径>，先停止本项目服务再使用 --confirm V2_SERVICES_STOPPED。')
  process.exitCode = 1
} else {
  ...
  try {
    const result = await upgradeV2GuestsFrom22To23(database, {
    ...
    console.log(`游客与正式轮次升级完成：schema=${result.previousSchemaVersion} -> ${result.schemaVersion}，resetEpoch=${result.resetEpoch}，备份 SHA-256=${result.backupSha256}`)
  } catch (error) {
    reportCliFailure('游客与正式轮次 schema 22→23 升级', error)
  } finally {
