...
if (!backupArgument) {
  console.error('V2 schema 12→13 升级失败：必须提供 --backup <未存在的 SQLite 备份路径>')
  process.exitCode = 1
} else {
  const config = loadConfig()
  const database = openDatabase(config.databasePath)
  try {
    const result = await upgradeSyntheticV2DatabaseFrom12To13(database, {
    ...
    reportCliFailure(
      `V2 schema 12→13 升级（确认值必须为 ${V2_DESTRUCTIVE_CONFIRMATION}）`,
      error,
    )
  } finally {
