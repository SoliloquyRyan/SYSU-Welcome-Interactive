...
D-023 的 G5-03～G5-05 已完成：`0005`/`0006` 分别加入 Kelvin 星色锁定与时光胶囊候选状态，`0007` 把旧六位码摘要原子替换为合成学号摘要；种子清单升级会保留邀请令牌和身份 ID。参与者接口支持个性令牌直接建会话及姓名+合成学号备用核验；后台可人工选择、上屏和撤下胶囊，大屏只获得最多 6 条严格脱敏的公开投影。字段处置见 [`../docs/G5_MOBILE_BASELINE.md`](../docs/G5_MOBILE_BASELINE.md)。
