...
- 四份有顺序、带校验和的 SQLite 迁移，以及 WAL、外键和忙等待设置；
- 300 个虚构参与者、固定虚构节目和四档礼物的本地种子；
- 随机邀请令牌、六位 Demo 码和后台口令只保存在被忽略的本地 manifest，数据库仅保存安全摘要；
...
D-023 已将六位 Demo 码和旧未来寄语列为 G5 待替换字段。迁移必须用合成学号摘要、Kelvin 星色锁定和时光胶囊人工候选状态原子替换全部数据库、种子、契约、接口和测试消费者；在迁移完成前不得只删除列或把本 README 改写成已实现。字段处置见 [`../docs/G5_MOBILE_BASELINE.md`](../docs/G5_MOBILE_BASELINE.md)。
