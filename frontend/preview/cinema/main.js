...
import GiftSkyEffects from '../../src/components/GiftSkyEffects.vue'
...
createApp({
  components: { ArrivalCount, GiftSkyEffects, PersonalJourneyStage, MobileBarrage },
  setup() {
    ...
    function starship() { flightVisible.value = true; clearTimeout(flightTimer); flightTimer = setTimeout(() => { flightVisible.value = false }, 2400 + 200); flight.value++ }
    async function inspectFlight() {
      clearTimeout(flightTimer); flightVisible.value = true; await nextTick()
      const elapsed = Number(flightProgress.value) / 100 * 2400
      document.querySelector('.gift-sky')?.getAnimations({ subtree: true }).forEach(animation => { animation.pause(); animation.currentTime = elapsed })
    }
  ...
  template: `<div class="review" :class="{ 'is-phone':phone }">
    ...
    <GiftSkyEffects v-if="flightVisible" :key="flight" :compact="phone" :reduced="staticMode" :effects="[{id:String(flight),giftId:'gift-starship',anchor:2,color:'#A4D7F3',quantity:2,batches:1,duration:2400}]"/><p v-if="error" class="error">{{error}}</p></main>
