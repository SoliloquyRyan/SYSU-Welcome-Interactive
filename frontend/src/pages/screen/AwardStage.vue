...
<template>
  <section class="ceremony-stage" :class="{ 'is-static': reduced, 'is-paused': paused }" :data-stage-mode="stage.mode" aria-label="颁奖与主持舞台">
    <ProgramStageBackground :stars="stars" :reduced="reduced" :paused="paused" :theme="stage.mode === 'AWARD' ? 'award' : 'host'" :audio="audio" :branded="false" @audio-status="$emit('audio-status', $event)" />
    <StageCollegeBrand />
    ...
    <footer v-if="stage.mode === 'AWARD' || title" class="ceremony-footer"><span>迎新之夜</span><span>二〇二六</span></footer>
  </section>
</template>
...
.ceremony-kicker{margin:0 0 2.2vh;font:500 1.3vw/1.6 var(--font-family-ui, sans-serif);letter-spacing:.38em;color:#8eafca}
...
h1.with-winners{font-size:3.3vw;letter-spacing:.1em}.ceremony-description{margin:1.5vh 0 0;color:#a5b8cb;font:400 1.35vw/1.6 var(--font-family-ui, sans-serif);letter-spacing:.13em}
...
.ceremony-stage{color:#f2eafa}
.ceremony-kicker,.award-rank,.award-page,.host-mark{color:var(--city-accent,#d4c0e7)}
.ceremony-description,.award-winners p,.ceremony-footer,.host-subtitle{color:#d0c3dc}
.ceremony-stage[data-stage-mode="AWARD"] .award-emblem span{color:#edd2a6;text-shadow:0 0 45px #c9a58225}
.ceremony-stage[data-stage-mode="AWARD"] .award-emblem i{border-color:#d8bda64f}
