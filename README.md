...
我曾经很在意自己是不是足够理性，能不能在意外发生时保持稳定。昨晚开场的网络故障让我很挫败，也让我重新认识到：现场系统需要为故障准备，项目负责人也可以承认一场演出没有按预期开始。断网首先是基础设施问题，接下来要做的是把事实记清、把系统补强，让下一次晚会拥有更可靠的起点。
...
公开仓库只放源码、脱敏示例配置、测试和流程文档。正式姓名、学号、令牌、密码、数据库、NFC 映射、OBS 凭据、Cookie、聊天导出和含身份信息的截图或录屏不得进入 GitHub、日志或普通协作盘。
