# 贡献指南

> **当前交接口径（2026-09-20）**：主线为 `main`。历史 D-072～D-109 文档中出现的功能分支名称已经合并并删除；新改动从 `main` 开始，并以 [`docs/README.md`](./docs/README.md) 的当前交接顺序为准。当前必需 CI 与历史 E2E 的边界见 [`docs/CI_STATUS.md`](./docs/CI_STATUS.md)。

> **D-107（2026-09-16，已发布）**：已认可的 D-102～D-106 更新已推送 GitHub 并部署至既有服务器。schema 21、现有场次与数据保留；36 段主持人动画及 25 项目录对稿页已上线；发布结果见 [D-107](./docs/D107_RELEASE_AND_HOST_CUES.md)，对稿说明见 [主持人对稿单](./docs/HOST_CUE_GUIDE.md)。下方旧版本状态保留历史时点。

> **当前 D-100（2026-09-13，已发布）**：全部已完成更新已推送 GitHub 功能分支，并部署至 `/welcomeparty/`，线上 schema 21。原测试轮次与 21 项目录保留；25 项新预设可在下一轮待开始时载入。公开资源、三端页面、后台登录及大屏实时连接已核验，其他项目保持运行。发布、备份和验证见 [D-100](./docs/D100_SERVER_AND_GITHUB_RELEASE.md)。下方版本状态均为当时历史记录。

> 当前代码目标为 D-097 / schema 21，线上以 D-095 / schema 19 发布记录为准。D-061 在 2026-09-06 的 schema 15 说明只代表当时实施点；已有持久库必须按其精确版本选择 [`运行手册`](docs/RUNBOOK.md) §7.6～§7.8 的停服、备份和专用升级入口，不能直接启动新代码或运行通用迁移。

感谢参与 SYSU Welcome Interactive。请先通过 Issue 明确需求、负责人、优先级、所属模块和验收标准，再开始实现。

协议 v2 的当前权威是 [`docs/DECISIONS.md`](./docs/DECISIONS.md) 与 [`docs/PROTOCOL_V2.md`](./docs/PROTOCOL_V2.md)；项目现状、文档地图与“下一步做什么”见 [`docs/README.md`](./docs/README.md)。D-037 已取消寄语/时光胶囊并启用节目中场个人抽奖；D-046～D-055 约束大屏银河、超新星接管、跨端背景和 220 人视觉基准；D-056 要求正式数据永不进 Git，生产只用仓库外私有目录、单个 loopback 后端、HTTPS/Secure Cookie 和一致性备份。D-067～D-071 已继续更新节目媒体叠加、聊天星号、手机布局、礼物档案和双端星舰。D-036 是变更前的已签核历史基线，不能代签当前版本。任何旧 `V2_ACTIVE` 库都必须在停服、独立备份和明确确认后按精确 schema 选择 [`docs/RUNBOOK.md`](./docs/RUNBOOK.md) 的专用升级入口；新工作以最新决策为准，先写 DECISIONS 再改代码。

## 开发环境

- Node.js 22.18.0 或兼容的更新版本
- pnpm 11.16.0

仓库使用根级 pnpm workspace 和唯一的根锁文件。请使用以下命令：

```bash
pnpm install --frozen-lockfile
pnpm dev:rehearsal              # 推荐给协作者；不需要也不得获取正式数据库
```

提交前从仓库根目录验证共享契约、后端、自动化测试与前端构建（快速门；收口用 `pnpm verify:v2-09`，完整命令清单见 [`docs/README.md`](./docs/README.md)）：

```bash
pnpm verify:g2
```

D-036 时点的三浏览器、300 人协议负载、30 分钟渲染 soak 和 V2-10 人工门均已通过；D-037 改变了入场、奖励、中场大屏和后台操作，D-048～D-050 更新了帧率、GPU 预热、跨端色谱与手机背景，D-051 又整体替换了集结→节目的黑洞路线，因此必须重跑适用自动门，并按 D-052 在至少一台代表实际上线访问方式的实体智能手机上重新人工验证当前入场、三端局域网抽奖与 OBS 合成。品牌、型号和操作系统不作阻塞限制，桌面移动端模拟不代签真机。既有结果只证明其明确记录的提交与范围，不能自动扩张到后续产品变更。

涉及三端页面、浏览器恢复或 G3 验收的变更，还应在 Windows Chrome、Edge 和隔离 Chromium 环境执行：

```bash
pnpm exec playwright install chromium
pnpm verify:g3
```

浏览器报告只保留脱敏的环境、用例名称、状态与耗时；不要开启或提交含邀请令牌、Demo 码或学号、Cookie、密码、身份映射、弹幕正文的 trace、截图、录屏和网络附件。

涉及实时协议、快照扇出、并发写入、恢复或 G4 性能边界的变更，先运行独立专项；准备收口 G4 时再运行包含 G3 全门的根命令：

```bash
pnpm test:load
pnpm verify:g4
```

`pnpm test:load` 使用 300 个固定合成参与者和独立临时数据库。生成的运行报告只允许保留脱敏汇总，并继续由 Git 忽略；不得提交凭据、姓名、完整学号、抽奖身份映射、完整弹幕正文、数据库或逐请求载荷。G4 的本机 loopback 结果不得扩张为分布式局域网、场馆、30 分钟 soak 或正式上线结论。

D-021 的提交前工作树验收见 [`docs/archive/acceptance-g0-g4.md`](./docs/archive/acceptance-g0-g4.md)：根级 `pnpm verify:g4` 退出码为 0，Vitest 20 个文件/119 条、共享契约/后端/前端构建，以及 9 场景 × 3 浏览器项目的 Playwright 27/27 均通过。该历史结果不绑定任何提交。G0～G4 冻结候选提交形成后，必须在该提交的干净工作树重跑适用门禁，并由 D-022 记录被验收提交的完整 SHA、命令结果与复验前后工作树状态；D-022 落地前只能称为冻结候选，不能称为可复现冻结版本。任何代码改动仍须重新执行适用门禁，不能长期借用 D-021 结果。

如果依赖发生变化，请更新对应 workspace 的 `package.json` 与根 `pnpm-lock.yaml`。不要提交 `node_modules/`、`dist/`、`backend/.data/`、`backend/.rehearsal/`、`backend/.private/`、正式备份、测试报告、本地环境文件或编辑器临时文件。

协议 v2 还遵守以下硬边界：

- 不建立 v1/v2 双写、静默状态翻译或失败后回退六阶段的兼容旁路；
- 数据库变更只能新增下一编号迁移，不修改已冻结的 `0001`～`0013`；
- 破坏性切换、升级或维护重置只允许在明确确认全部数据均为可丢弃合成/Demo 数据后执行；发现真实、不可重建或有保留价值的数据必须停止，不能自动清空；
- 不因本机已安装技能而增加 GSAP、Three.js 或其他运行时依赖；任何新依赖都需要独立授权、范围、性能和降级审查。
- 不把正式数据库发给协作者。功能、UI、协议和压力修改使用 `pnpm dev:rehearsal` 的固定 300 人合成技术目录，视觉验收仍以 220 人为正式参考；生产部署按 [`docs/SERVER_DEPLOYMENT.md`](./docs/SERVER_DEPLOYMENT.md) 执行。

## Issue 流程

新功能和缺陷分别使用对应 Issue 模板。每个 Issue 必须包含：

- 明确的负责人或“待认领”；
- P0–P3 优先级；
- 所属模块；
- 背景、复现信息或建议方案；
- 可检查的验收标准。

P0 表示阻断交付或现场核心流程，需优先同步项目负责人；P1 为高影响事项；P2 为常规需求；P3 为低优先级改进。

## 分支与提交

通常从最新的 `main` 创建短生命周期分支，建议使用：

- `feature/<简短名称>`
- `fix/<简短名称>`
- `docs/<简短名称>`
- `release/<版本名称>`（冻结版本，如 `release/demo-v0`）

已记录的例外：`feature/demo-v0` 根据 `docs/DECISIONS.md` 的 D-006，从 `design/invitation-ui-guide` 创建，以继承尚未合并的三端视觉基线；不得为此切换到 `main`、新建仓库或额外工作区。该例外不改变其他功能分支的默认规则。

提交应聚焦单一目的，说明“做了什么”和“为什么”。不要把格式化、依赖升级和功能修改无关地混在同一个提交中。

## Pull Request

创建 PR 时请完整填写模板，至少包括：

- 关联 Issue；
- 变更说明；
- 界面截图或录屏（可见改动）；
- 已执行的测试与构建命令；
- 数据、接口、本地存储及兼容性影响；
- 可执行的回退方案；
- 无敏感信息确认。

PR 必须通过 CI 的冻结依赖安装、类型检查、自动化测试和生产构建。评审意见处理完成后再合并；未解决的高优先级问题不得以备注代替修复。

## 数据与安全

- 仅采集实现功能所必需的数据，并避免在页面、日志、截图或大屏中暴露姓名、学号等个人信息。
- 不得提交访问令牌、API 密钥、密码、私有链接或真实个人数据。
- 示例、测试和截图必须使用虚构或脱敏数据。
- 发现凭据泄露时，不要在公开 Issue 中粘贴内容；应立即通知负责人并轮换凭据。
