...
> 状态：D-022 当前运行契约（v0.6）仍可用；D-023 的 G5 身份、星色和时光胶囊迁移尚未实现，目标边界见第 8 节。
...
| 方法与路径 | 作用 |
|---|---|
| `POST /api/participant/activate` | D-022 当前：以令牌、虚构姓名和六位 Demo 码核验；G5 将拆为令牌主入口与姓名+合成学号备用核验 |
...
3. D-022 当前恢复相同的固定合成身份、六位 Demo 码、节目和礼物；G5 迁移后改为恢复合成学号摘要、星号、节目和礼物，并清除星色锁定与胶囊候选；
