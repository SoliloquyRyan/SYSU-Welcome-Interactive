# Demo v0 API 边界（协议 v1 已实现快照）

> 状态：本文完整记录当前协议 v1 的已实现 API 与回归边界；G5 身份、星色、时光胶囊、后台人工选择及大屏安全投影均已在 v1 实现。
>
> v2 边界：D-025/D-026 与 [`PROTOCOL_V2.md`](./PROTOCOL_V2.md) 是当前产品/协议权威。V2-01～V2-07 已实现共享契约、握手、业务域、三类权威快照、ACTIVE 分流实时层、匿名后台与大屏；仓库实际 `.data` 仍运行 v1，手机仍待 V2-08。处于 `V2_ACTIVE` 的合成库开放 `GET /api/v2/screen|participant|admin/snapshot`、`POST /api/v2/participant/activate`、`POST /api/v2/participant/commands`、`POST /api/v2/admin/commands` 和 `/ws/v2`。弹幕/礼物公共事件及审核命令也统一走共享命令/事件联合，不新增身份化大屏端点；完整字段以共享 `protocol-v2.ts` 与权威协议为准。本文中的 `stage=1..6`、旧路径和字段只能作为 v1 回归参考，不得据此自行发明兼容路由或双写层。
...
- 屏幕快照和公共事件不得包含姓名、时光胶囊正文、完整邀请令牌、合成学号、Cookie、后台密码或会话凭据。
...
### 1.1 V2-01 握手端点（契约就绪，业务未激活）

- `GET /api/protocol-capabilities`：版本中立能力发现；当前返回 `contractVersion='2'`、`activeRuntimeVersion='1'`、`activationState='CONTRACTS_READY'`，三项 v2 业务能力为 false。
- `POST /api/v2/handshake`：严格接受 `protocolVersion='2'`、三端 `clientSurface` 与非空 `clientBuild`；错误版本和畸形字段显式拒绝。
- `/ws/v2`：当前只接受严格 `HELLO`，发送 `HELLO_ACK` 与 `NOT_ACTIVE` 后以 1013 关闭；非法协商以 `ERROR` 和 1008 关闭。V2-05 前不发送业务快照或实时业务事件。

完整字段、生命周期和 V2-05 边界见 [PROTOCOL_V2.md 第 1.1 节](./PROTOCOL_V2.md#11-v2-01-已实现的协议握手层)。

## 2. v1 会话与角色（历史回归）

### 2.1 参与者会话

- 有效个性邀请令牌是主入口并可直接建立会话；姓名与合成学号只用于无法使用邀请函时的备用核验。
...
G2/G5 的 v1 实现保留下列精确路径。除登录、登出与只读接口外，写命令必须携带 `Idempotency-Key`。这些路径描述当前可运行的 v1，不预先承诺 v2 路径、请求体或兼容行为。
...
- D-022 旧未来寄语按 Unicode 码点计算，最多 80 个可见字符；G5 时光胶囊默认继承该上限，并新增公开范围确认和人工候选状态。
...
3. D-023 当前恢复相同的固定合成身份、合成学号摘要、星号、节目和礼物，并清除星色锁定与胶囊候选；D-022 的六位 Demo 码仅作为历史验收事实保留；
...
D-023 曾确认 v1 产品语义，G5-03～G5-05 已原子完成并验证以下替换；这些结果是 v2 切换前的回归起点。V2-01 之后的迁移仍不得只改数据库列或只改前端文案：
