...
- **REQ-V2-MOTION-004**（AUTO-PASS）：D-027～D-032 的聚焦浏览器自动门均已通过；vivo X300 人工复验仍 `PENDING`。

### 3.6 数值与重置（PROTOCOL_V2 §8/§10）

- **REQ-V2-VALUE-001**：星光 = 激活 20、胶囊首次实际提交 20、星星启动 20、首次送礼 10、首次合规弹幕 10、协同点亮 20；跳过 0；上限 100。
- **REQ-V2-VALUE-002**：奖励规则服务端版本化（`rewardRuleVersion`），每行账本保存实际增量与版本；调整只从新 epoch 生效。
...
- **REQ-V2-RESET-003**：v1→v2 只允许一次性合成数据切换（`0008` 迁移）；失败整体恢复 v1 备份，不混跑。

## 4. 待确认汇总

1. 正式到场规模、节目单、场地、时间与舞台 Cue。
2. 真实身份字段、名单来源、访客模式与人工核验。
3. 胶囊大屏版式、轮播数量、撤下方式、录像/直播提示与保存期限。
