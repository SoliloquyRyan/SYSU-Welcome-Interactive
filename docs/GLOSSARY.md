...
| 术语 | 含义 |
|---|---|
| formation slot | 身份目录内预分配的不透明稳定星位；单场容量上限 300，首次激活在同一事务中原子预留，满员整笔拒绝 |
...
| `publicStarId` | 锁色后成为公共恒星主键；合成 Demo 沿用历史格式，正式 `PROTECTED` 目录使用与姓名/学号无关的随机 `A-0000` 形态 |
...
| `PROTECTED` | 从获授权名单新建、带源摘要证据且不可用 Demo 重置的正式数据分类；与 `SYNTHETIC_DEMO` 物理隔离 |
...
| `rewardRuleVersion` | 星光规则版本；D-037/D-054 的 schema 12→14 维护一次性转换旧合成账本并增加受保护来源能力，升级后同一 epoch 不得热切换 |
...
| 维护门 | v1→v2 一次性切换、schema 12→14 升级、合成重置与受保护名单初始化的操作门槛（RUNBOOK §7、PROTOCOL_V2 §10） |
| 合成数据 | 固定种子的虚构身份与节目，只用于测试/预演；获授权真实数据仅可进入 Git 忽略的受保护运行通道，不得进入仓库与公开证据 |
