...
> **当前主线（2026-09-20）**：公开仓库只保留 `main`，当前提交以 GitHub `main` 为准。D-072～D-109 等记录中的 `feature/screen-six-stage-experience` 是当时的历史工作分支，已经合并并删除；这些记录用于追溯当时的决策和验证，不代表当前分支或当前线上状态。

> **接手顺序**：先看[下一届现场一页启动单](./NEXT_YEAR_QUICKSTART.md)，再看[赛后审阅与下一届改进包](./POST_EVENT_REVIEW_20260920.md)、[主控操作单](./LIVE_OPERATOR_GUIDE.md)和[主持对稿单](./HOST_CUE_GUIDE.md)。历史 D 文档、旧部署地址和旧 schema 只作证据，不要直接照搬。

> **CI 口径**：必需 CI 运行当前 D-119 现场 E2E smoke；仓库保留的旧 D-106/D-109 E2E 仍待按当前流程迁移。`pnpm test:e2e:all` 可手动运行完整历史矩阵，不能把其中的红基线写成当前现场故障。

## 赛后交接（2026-09-20）

- [赛后审阅与下一届改进包](./POST_EVENT_REVIEW_20260920.md)：流程亮点、现场网络故障复盘、原始创意代码落点和优先级。
- [下一届现场一页启动单](./NEXT_YEAR_QUICKSTART.md)：发布前检查、断网演练、角色分工和停止条件。

> **D-118（2026-09-19，已发布，现场设备待复核）**：最终联动修正：游客转学生核验使用独立手机布局；OBS cue 增加 HOST 舞台模式并固定切换 `00 开场与集结`；颁奖公共屏幕只显示已揭晓姓名；互动控制台合并报幕、执行、音频和收尾操作。生产版本 `20260919-d118-bbd942f108cb` 已原子切换，包 SHA-256 `bb460ab26bcc6c2a8387a0d69831a0459a11b8108c97161f900f185d8c3e3e05`。详见 [D-118 实施记录](./D118_IMPLEMENTATION.md)。

> **D-117（2026-09-19，已发布，现场设备待复核）**：网页切换节目后，本机 OBS 桥接按 `programId` 映射自动同步 OBS 场景；断线恢复会按当前节目重试，未知映射不盲切。生产版本 `20260919-d117-23676e22f400` 已上线，本机 OBS 自动恢复实测通过。详见 [D-117 实施记录](./D117_IMPLEMENTATION.md)。

> **D-116（2026-09-19，已发布，现场设备待复核）**：互动一由本机 OBS 桥接自动播放 `mix`，播放开始后服务器倒数 10 秒；后台显示附件最新版答案。OBS 已移除旧银河、旧完整舞台和学院静态源，统一黑底与最新版网页背景；半屏控台、弹幕窄屏布局、节目单和校园奖名单已完成发布核验。详见 [D-116 实施记录](./D116_IMPLEMENTATION.md)。

> **D-114（2026-09-18，本地实施）**：受保护账号核验包已写入项目外目录，游客继续动态创建；手机统一使用竖版雾紫城市背景，入场不创建音乐，游客入口紧贴确认按钮；已生成 48 页手动翻页断网舞台 PPT/PDF。账号与离线包位置、验证范围见 [D-114 实施记录](./D114_IMPLEMENTATION.md)。本轮不部署、不重置、不新增 schema。

> **D-112（2026-09-18，已发布）**：修复手机端启动空白和未登录背景渲染循环；主持人对稿页仅保留本机离线包，服务器路径返回 404。schema 24、真实名单测试轮次和 D-111 恢复点保持不变。发布与核验见 [D-112](./D112_IMPLEMENTATION.md)。

> **D-110（2026-09-18，本地验收版）**：手机所有页面统一为低饱和竖版雾紫背景，入场只播放个人流星；大屏取消爆发转场，结束后直接显示“感谢你的参与”；互动一改为 OBS `mix` 开始后服务器倒数 10 秒自动开放抢答，答错续播。schema 24 与 D-109 正式数据兼容，本轮不自动重置、不发布服务器。当前实现与边界见 [D-110](./D110_IMPLEMENTATION.md)，主持口令见[主控操作单](./LIVE_OPERATOR_GUIDE.md)和[主持对稿单](./HOST_CUE_GUIDE.md)。
...
> **D-102（2026-09-15，本地清理）**：已从 D-100 发布基线审计冗余代码和产物，重要版本移入可校验历史归档，当前服务器仍为 D-100。见 [清理审计](./D102_CODE_AND_ARTIFACT_CLEANUP.md) 与 [历史参考索引](./archive/visual-assets/README.md)。下方各阶段记录保留当时状态。
