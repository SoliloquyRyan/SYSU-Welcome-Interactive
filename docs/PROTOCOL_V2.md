...
> **D-109（schema 23）现行补充**：以 [D-109 实施记录](./D109_IMPLEMENTATION.md)及当前 contracts 为准，下方早期容量、美术和抽人规则保留历史口径。256 学生＋50 工作人员为固定受保护名单；每轮允许最多 94 个游客，总容量 400。手机统一表单支持原核验方式附带星色，`GUEST` 方式提交昵称与星色；首次核验、初始额度、锁色原子提交，已入场者恢复原色与余额。游客持久恢复 Cookie 的摘要和创建幂等摘要绑定轮次，不以昵称或 IP 作为身份唯一依据。`account_kind` 是服务器权威身份类型，旧 `account_type` 保留用于 schema 22 名单指纹兼容；固定名单校验排除游客。工作人员和游客不计正式动力、不投票、不抢答。
>
> `RESET_FORMAL_ROUND` 只允许 ALL 主控，在 READY / PAUSED / COMPLETED 中携带当前轮次、版本、幂等键及“重新开场”确认执行。归档校验与并发写入屏障在事务前建立，新轮次事务保留名单、目录、校园奖内容并清除轮次状态；不可下载归档，管理员仅见时间、轮次和哈希。归档收据跨重置保留，原请求重试不再次重置。schema 22→23 升级保留现有轮次和历史，发布不调用重置命令。
>
> 互动 A 保留抢答；B 使用独立候选 ID / 标签，不依赖抽人；C 只线下开展。片尾从节目阶段直接确认 COMPLETE。手机结束纪念页保持只读订阅，以接收新轮次通知；重置后清除私人 DOM、重新核验与选色。公共大屏仍只包含公开星号、颜色、节目和已揭晓结果。
