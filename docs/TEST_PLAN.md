...
> **D-107（2026-09-16，已授权发布，执行中）**：负责人认可 D-106，授权将 D-102～D-106 全部已完成更新上传 GitHub 与既有服务器。保持 schema 21 和现有场次数据，交付主持人动画对稿页；发布结果见 [D-107](./D107_RELEASE_AND_HOST_CUES.md)，对稿说明见 [主持人对稿单](./HOST_CUE_GUIDE.md)。下方旧版本状态保留历史时点。

> **D-106（2026-09-16，本地调整，未发布）**：接入负责人提供的方线体标题，放大节目名称、表演者及视频报幕。完整背景的真实星只投影到素材留白；视频星光全屏微弱分布、收礼局部亮起。手机开放更多留白星空，增加明暗层次，四档礼物触发星点短暂闪亮并保留输入。仍按一人一星，不新增业务字段。当前规则与实际验证见 [D-106](./D106_WHITESPACE_STARS_AND_TYPE.md)，下方内容保留各自版本时点。

> **D-105（2026-09-16，本地版本，未发布）**：按最新反馈回到已认可的粉紫、青蓝饱和度，保留朦胧蒸汽质感；每个节目使用不同场景、透视与构图。一位已入场观众对应一颗真实星；10 套专属节目背景全程保留节目名和表演者，9 套已有媒体保持透明叠层。英文采用 Orbitron，抒情 / 器乐补充本地宋体子集，方线体文件及网页授权待提供。音乐层支持显式现场输入；业务与 schema 21 保持。当前规则及实际验收范围见 [D-105](./D105_AUDIENCE_PROGRAMS_AND_AUDIO.md)。下方旧版本规则仅保留历史时点。

> **D-104（2026-09-16，本地可验收版，未发布）**：在 D-103 构图上增加霓虹剪影、三类节目背景和装饰星；报幕使用纯主题，大屏采用得意黑／Oxanium 标题，手机统一本地分片 Noto；四档天空礼物替代大型星舰。公共礼物事件可选星色由服务器锁定值提供，schema 21 与账目保持。当前规则和验收范围见 [D-104](./D104_NEON_STARS_AND_GIFTS.md)，历史版本保留。

> **D-103（2026-09-16，本地实现与验收，未发布）**：后台采用 OBS 式分区控台和串行组合操作；大屏与手机使用简化赛博朋克加微复古的雾紫星城，保留星光、透明叠加、个人星色及现有业务状态。新增本机 OBS 音量桥接，schema 21 与 D-100 发布基线、D-102 历史归档保持。当前规则、截图与实际验证范围见 [D-103](./D103_STAR_CITY_UI.md) 和 [主控操作单](./LIVE_OPERATOR_GUIDE.md)。下方旧决策保留各自时点范围。
