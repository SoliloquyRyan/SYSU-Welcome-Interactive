...
Windows 下直接双击仓库根目录的 `打开手机端预览.cmd`。该入口会自动进入仓库、查找系统或 Codex 自带的 `pnpm`，并仅在首次缺少依赖时执行冻结锁文件安装。启动后会同时打开 390×844 手机端和动画阶段控制台：手机端确认星色后，在控制台选择阶段 1～6并点击“跳转”，即可反复查看六阶段页面及其切换动画。
...
第二条命令会启动隔离的临时后端与真实前端，自动打开 `390×844` 手机端和桌面阶段控制台。控制台会使用仅存在于进程内存中的合成后台凭据自动登录、取得全部能力并启动排练，不在终端或页面中显示凭据。手机端完成备用核验并停在“选择你的恒星色温”；确认星色后，通过控制台任意跳转阶段 1～6。关闭手机端窗口或按 `Ctrl+C` 会关闭全部窗口、删除临时数据库并释放端口。
